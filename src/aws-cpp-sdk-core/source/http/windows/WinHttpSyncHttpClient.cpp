...
static void WinHttpSetHttpVersion(void* handle, const Aws::Http::Version version)
{
    ...
    if (!AzCallWinHttp("WinHttpSetOption", WinHttpSetOption, handle, WINHTTP_OPTION_ENABLE_HTTP_PROTOCOL, &winHttpVersion, sizeof(winHttpVersion)))
    {
    ...
    m_version(config.version)
{
    ...
    if (m_verifySSL)
    {
        ...
        if (!AzCallWinHttp("WinHttpSetOption", WinHttpSetOption, GetOpenHandle(), WINHTTP_OPTION_SECURE_PROTOCOLS, &flags, sizeof(flags)))
        {
...
#endif
        if (!AzCallWinHttp("WinHttpSetOption", WinHttpSetOption, hHttpSession, WINHTTP_OPTION_TCP_KEEPALIVE, &keepAlive, sizeof(keepAlive)))
        {
...
void* WinHttpSyncHttpClient::OpenRequest(const std::shared_ptr<HttpRequest>& request, void* connection, const Aws::StringStream& ss) const
{
    ...
    if (!m_verifySSL) // Turning ssl unknown ca verification off
    {
        ...
        if (!AzCallWinHttp("WinHttpSetOption", WinHttpSetOption, hHttpRequest, WINHTTP_OPTION_SECURITY_FLAGS, &flags, sizeof(flags)))
        {
    ...
    if (!GetConnectionPoolManager()->GetEnableTcpKeepAlive())
    {
        ...
        AzCallWinHttp("WinHttpSetOption:WINHTTP_DISABLE_KEEP_ALIVE", WinHttpSetOption,
                      hHttpRequest, WINHTTP_OPTION_DISABLE_FEATURE, &keepAliveDisable, sizeof(keepAliveDisable));
    }
    ...
    if (!m_allowRedirects)
    {
        ...
        if (!AzCallWinHttp("WinHttpSetOption", WinHttpSetOption, hHttpRequest, WINHTTP_OPTION_DISABLE_FEATURE, &requestFlags, sizeof(requestFlags)))
        {
...
{
    bool success = WinHttpSendRequest(hHttpRequest, NULL, NULL, 0, 0, 16 * 1024, NULL) != 0;
    if (!success)
    {
